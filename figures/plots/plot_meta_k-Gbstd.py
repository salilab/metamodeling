...
# Read data file
with open("../../bnet/meta_meal_normal_variables_k-Gb_sigma.txt", 'rb') as kfile:
    next(kfile)
    data = list(csv.reader(kfile))
    kfile.close()

with open("../../bnet/meta_spt_normal_variables_k-Gb_sigma.txt", 'rb') as kfile:
    next(kfile)
    data2 = list(csv.reader(kfile))
    kfile.close()
    
Gb_posterior_mu= (np.linspace(0,0.01,20))**0.5
...
ax1.set_xlabel('standard deviation of G$_{basal}$ (mM)',fontname="Arial",fontweight="normal",fontsize="20")
ax1.set_ylabel('k.SPT',fontname="Arial",fontweight="normal",fontsize="20")
ax1.tick_params(direction='in', pad=6)
ax1.xaxis.set_label_coords(0.5, -0.1)
ax1.yaxis.set_label_coords(-0.1, 0.5)
ax1.set_xlim([0, 0.01])
ax1.set_ylim([10.5,11.5])
...
fig.savefig("meta_spt_k-Gbstd.png",dpi=300)
