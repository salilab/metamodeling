...
for i in range(0, len(meal)):
    meal_G.append(float(meal[i][21]))
    meal_Gstd.append(float(meal[i][23]))
    meal_I.append(float(meal[i][39]))
    meal_Istd.append(float(meal[i][41]))
...
ytick_locs=[0,20,40,60]
ytick_lbls=['0','20','40','60']
...
fig.savefig("spt_normal.png", transparent=True,dpi=400)
