...
# Read data file
with open("../../bnet/meta_meal_normal.txt", 'rb') as kfile:
    next(kfile)
    meal = list(csv.reader(kfile))
    kfile.close()

with open("../../bnet/meta_spt_normal.txt", 'rb') as kfile:
    next(kfile)
    spt = list(csv.reader(kfile))
    kfile.close()

with open("../../bnet/meta_network_normal.txt", 'rb') as kfile:
    next(kfile)
    network = list(csv.reader(kfile))
    kfile.close()
...
for i in range(0, len(meal)):
    ...
    spt_I.append(float(spt[i][19]))
    spt_Istd.append(float(spt[i][21]))
    network_I.append(float(network[i][13]))
    network_Istd.append(float(network[i][15]))
...
ax1.set_ylabel('Gex (mM)',fontname="Arial",fontweight="normal",fontsize="20",color='chocolate')
...
# Plot for heating
ax1.errorbar(timeslice,meal_G, yerr=meal_Gstd,marker='.',linestyle='none',markersize=2, fmt='-',capsize=1.2, elinewidth=0.5,linewidth=0.5,c='grey',alpha=0.6)
ax1.plot(timeslice,meal_G,linestyle='-',c='grey',linewidth=1)

ax2.errorbar(timeslice,meal_I, yerr=meal_Istd,marker='.',linestyle='none',markersize=2, fmt='-',capsize=1.2, elinewidth=0.5,linewidth=0.5,c='k',alpha=0.6)
ax2.plot(timeslice,meal_I,linestyle='-',c='k',linewidth=1, alpha=0.6)
...
ax1.plot(timeslice,meta_meal_G,linestyle='-',c='brown',linewidth=1)
...
ax2.plot(timeslice,meta_meal_I,linestyle='-',c='darkgreen',linewidth=1, alpha=0.6)
...
ax1.plot((315, 335),(28.5,28.5),linestyle='-',c='grey',linewidth=1.5)
ax1.plot((315, 335),(26.5,26.5),linestyle='-',c='black',linewidth=1.5)
ax1.plot((315, 335),(24.5,24.5),linestyle='-',c='chocolate',linewidth=1.5)
ax1.plot((315, 335),(22.5,22.5),linestyle='-',c='darkgreen',linewidth=1.5)

ax1.plot((0, 420),(5.1, 5.1),linestyle='-',c='grey',linewidth=1)
ax2.plot((0, 420),(25, 25),linestyle='-',c='black',linewidth=1)
...
ax1.text(0.82*(left+right), 0.81*(bottom+top), 'Gex.Meta',horizontalalignment='left',verticalalignment='center',fontsize=18,fontname="Arial",fontweight="normal",color='k',transform=ax1.transAxes)
ax1.text(0.82*(left+right), 0.74*(bottom+top), 'I.Meta',horizontalalignment='left',verticalalignment='center',fontsize=18,fontname="Arial",fontweight="normal",color='k',transform=ax1.transAxes)
...
fig.savefig("meta_GS_normal_Gobs.png", transparent=True,dpi=400)
