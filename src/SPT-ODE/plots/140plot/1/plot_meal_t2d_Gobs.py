...
with open("../../bnet/meal_G-S_t2d.txt", 'rb') as kfile:
    next(kfile)
...
for i in range(0, len(data)):
    ...
    S.append(float(data[i][2]))
    Sstd.append(float(data[i][3]))
...
ax1.plot(timeslice,G,linestyle='-',c='brown',linewidth=1.5)

ax2.plot(timeslice,S,linestyle='none',marker='.',markersize=4,markerfacecolor="None",markeredgecolor='green', markeredgewidth=1, alpha=0.6)
ax2.errorbar(timeslice,S, yerr=Sstd,marker='.',linestyle='none',markersize=2, fmt='-',capsize=1.4, elinewidth=1,linewidth=1,c='green',alpha=0.6)

Sfit1 = savgol_filter(S, 139, 15) # window size 51, polynomial order 3

ax2.plot(timeslice[0:50],Sfit1[0:50],linestyle='-',c='darkgreen',linewidth=1.5)
ax2.plot(timeslice[49:],S[49:],linestyle='-',c='darkgreen',linewidth=1.5)
...
fig.savefig("meal_GS_t2d_Gobs.png", transparent=True,dpi=400)
