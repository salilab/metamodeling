...
with open("../../bnet/meal_G-S_normal.txt", 'rb') as kfile:
    next(kfile)
...
for i in range(0, len(data)):
    ...
    #Gstd.append(float(data[i][2]))
    Gstd.append(0.01)
    I.append(float(data[i][6]))
    #Istd.append(float(data[i][8]))
    Istd.append(0.01)
...
ax2.set_ylabel('I.Meal (pM)',fontname="Arial",fontweight="normal",fontsize="20",color='green')
...
ax2.plot(timeslice,I,linestyle='none',marker='.',markersize=4,markerfacecolor="None",markeredgecolor='green', markeredgewidth=1, alpha=0.6)
...
ax1.plot((0, 420),(5.1, 5.1),linestyle='--',c='brown',linewidth=1)
...
fig.savefig("meal_GS_normal_Gobs.png", transparent=True,dpi=400)
