...
with open("../bnet/meta_k_normal.txt", 'rb') as kfile:
    next(kfile)
...
for i in range(0, len(data)):
    ...
    prior_k_mu.append(float(data[i][9]))
    prior_k_sigma.append(float(data[i][10]))
    prior_k_std.append(float(data[i][11]))
...
for i in range(130,131):
    prior_k = gaussian(x, spt_k_mu[i], spt_k_sigma[i])
    ...
    print(prior_k_mu[i], prior_k_std[i])
...
fig.savefig("meta_k_normal.png",dpi=300)
