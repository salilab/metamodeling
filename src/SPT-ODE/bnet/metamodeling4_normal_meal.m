...
for measure = 1:420
    ...
    evidence{nodes_map('GLP1a.obs'),measure} = 0; % evidence at time slice 2
    evidence{nodes_map('cons.obs'),measure} = 0; % evidence at time slice 2
   
    meal_evidence{meal_nodes_map('DGintake.obs'),measure} = DGexp(measure); % evidence at time slice 2
    glp1r_evidence{glp1r_nodes_map('GLP1a.obs'),measure} = 0; % evidence at time slice 2
    glp1r_evidence{glp1r_nodes_map('cons.obs'),measure} = 0; % evidence at time slice 2
end
...
writetable(T, 'meta_meal_normal.txt');
...
T1 = table(spt_Gin, spt_k, spt_Npatch, spt_Nisg, spt_Ninsulin, spt_S, spt_I,meta_spt_Gin, meta_spt_k, meta_spt_Npatch, meta_spt_Nisg, meta_spt_Ninsulin, meta_spt_S, meta_spt_I);
% Write data to text file
writetable(T1, 'meta_spt_normal.txt');
...
% Write data to text file
writetable(T2, 'meta_network_normal.txt');
