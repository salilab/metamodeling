...
% meta model
Json_meta = jsondecode(fileread('../data/meta_normal_fig4.json'));
% Gb_k_input = importdata(Json_meta.InputErr);

Gpl_c_weight_list = importdata(Json_meta.G_pl_w);
Gpl_c_cov_list = importdata(Json_meta.G_pl_cov);

for v = 1:11
    G_pr_weight = v*0.1-0.4;
    for k = 1:length(Gpl_c_cov_list(:,1))
        Gpl_c_cov = Gpl_c_cov_list(k,1); 
    
        [dbn, nodes_map]= make_meta_dbn3_Gpl(Json_postprandial.DataInput.DGd_mean_postprandial, Json_postprandial.DataInput.DGd_cov_postprandial, Json_postprandial.DataInput.Gb_mean_postprandial,...
                                         ...
                                         Json_meta.DataInput.S_ve_weight, G_pr_weight, ...
        ...
        fname = sprintf(strcat('../Output/fig4_test_Gpl/metamodel_normal_w_',num2str(G_pr_weight),'_cov_', num2str(Gpl_c_cov), '.csv'));
        writetable(T, fname);
    end
