...
% SVE = importdata(Json_meta.EvidenceInput.Evidence_Scell);
% for measure = 1:Json_meta.DataInput.T
%    evidence{nodes_map('Sobs.exocytosis'), measure} = SVE(measure);
% end
...
fn = ['../Output/metamodel_normal_obs_test_conflict.csv'];
fname = sprintf(fn);
writetable(T, fname);
