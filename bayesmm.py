# Import libraries:{{{
import os, math, re, warnings, time, copy
import numpy as np
import pandas as pd
import torch
...
class BayesMM:
    ...
    def inference( self, data_list, couplings, steps, params_list=None, M2_interp=None, progress_bar=True):
        """
        ...
        for t in progress:
            ...
            for c_idx, c in enumerate(stat_couplings):
                ...
                for model_local, m in enumerate(c['models']):
                    ...
                    var_idx  = data_list[m].columns.get_loc(var_name) - 1
                    mu       = states[m][var_idx]
                    sigma    = torch.sqrt(torch.clamp(covariances[m][var_idx, var_idx], min=1e-10))
                    mus.append(mu)
                    sigmas.append(sigma)
    ...
    def compute_data_likelihood_energy(self, steps, z_pred_over_time, P_zz_over_time, data_list):
        total_nll_data = torch.tensor(0.0)
        for i, (z_pred_list, P_zz_list, data_df) in enumerate(zip(z_pred_over_time, P_zz_over_time, data_list)):
            state_dim = self.models[i].state_dim
            obs_names = data_df.columns[1:1+state_dim]
            obs_mean  = torch.tensor(np.stack([data_df[name] for name in obs_names], axis=1), dtype=torch.float32)
            for t in range(steps):
                dist = MultivariateNormal(z_pred_list[t], covariance_matrix=P_zz_list[t])
    ...
    def compute_state_variable_likelihood_energy(self, steps, states_over_time, Zi_over_time, P_pred_over_time):
        ...
        for x_up_list, x_pred_list, P_pred_list in zip(states_over_time, Zi_over_time, P_pred_over_time):
            for t in range(steps):
                dist = MultivariateNormal(x_pred_list[t], covariance_matrix=P_pred_list[t])
    ...
    def compute_loss(self, data_list, couplings, steps, Zi_over_time, states_over_time,
        covariances_over_time, z_pred_over_time, P_zz_over_time, P_pred_over_time,
            ...
            for t in range(steps):
                ...
                for c in couplings:
                    ...
                    for j, m in enumerate(c['models']):
                        ...
                        var_j    = torch.clamp(cur_covs[m][var_idx, var_idx].to(device=device, dtype=dtype), min=1e-12)
...
def build_metamodel(
    ...
    if connecting_vars is None:
        params_list_for_inf = [copy.deepcopy(m["params"]) for m in models_data]
