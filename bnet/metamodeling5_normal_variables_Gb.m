...
% 1. The meal model of normal subject
% 2. The SPT model
% 3. The network model
% 4. The GLP1R model
% 5. The KEGG model
...
% Sampling parameter:
% Evidence:
%   1. Scell.ref is set to the basal value (meta_meal_normal.txt) to allow flow of information for studying model
%   varaibles.
%   2. Gb in the meal model is set to different values to exam its effect in the meta-model.
% 
% Return:
...
Metafile = csvread('/Users/lipingsun/SynologyDrive/research/Meta-modeling/bnt/Metamodeling-bnet/bnet/meta_meal_normal.txt',1,0);
...
%Gb_Meal_sigma = 0.001; % sigma of Gb
...
% KEGG
Gculture_KEGG = 0 % Extracellular glucose concentration, mM 
...
for Gb_values = linspace(0,20,21)
    disp(Gb_values)
end
...
for Gb_values = linspace(0,20,21)
    for measure = 1:200
        disp(measure);
        %evidence{nodes_map('DGintake.obs'),measure} = 0; 
        ...
        evidence{nodes_map('Gb.Meal'),measure} = Gb_values; 
        evidence{nodes_map('Scell.ref'),measure} = Scell; 
        %evidence{nodes_map('Gex.ref'),measure} = Gex;
        %evidence{nodes_map('Gin.ref'),measure} = Gin; 
    
        %meal_evidence{meal_nodes_map('DGintake.obs'),measure} = 0; 
...
writetable(T, 'meta_meal_normal_variables_k-Gb.txt');
...
% Write data to text file
writetable(T1, 'meta_spt_normal_variables_k-Gb.txt');
...
% Write data to text file
writetable(T2, 'meta_network_normal_variables_k-Gb.txt');
