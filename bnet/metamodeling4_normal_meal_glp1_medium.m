...
for measure = 1:420
    ...
    evidence{nodes_map('GLP1a.obs'),measure} = 5; % evidence at time slice 2
    evidence{nodes_map('cons.obs'),measure} = 5; % evidence at time slice 2
   
    meal_evidence{meal_nodes_map('DGintake.obs'),measure} = DGexp(measure); % evidence at time slice 2
    glp1r_evidence{glp1r_nodes_map('GLP1a.obs'),measure} = 5; % evidence at time slice 2
    glp1r_evidence{glp1r_nodes_map('cons.obs'),measure} = 5; % evidence at time slice 2
end
...
writetable(T, 'meta_meal_normal_glp1_medium.txt');

% Create a table with the data and variable names
T = table(spt_Gin, spt_k, spt_Npatch, spt_Nisg, spt_Ninsulin, spt_S, spt_I, meta_spt_Gin, meta_spt_k, meta_spt_Npatch, meta_spt_Nisg, meta_spt_Ninsulin, meta_spt_S, meta_spt_I);
% Write data to text file
writetable(T, 'meta_spt_normal_glp1_medium.txt');

% Create a table with the data and variable names
T = table(network_Gin, network_cAMP, network_Ca, network_S, network_I, meta_network_Gin, meta_network_cAMP, meta_network_Ca, meta_network_S, meta_network_I );
% Write data to text file
writetable(T, 'meta_network_normal_glp1_medium.txt');
